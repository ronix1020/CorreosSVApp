    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.secondaryContainer}>
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          ...
          />
          <Text style={styles.titleTracking}>{name}</Text>
        </View>
        <Button
          mode="elevated"
        ...
        {response ? (
          <>
            <View style={styles.cardMainMessage}>
              <Text style={styles.textMainMessage}>Mensaje Principal</Text>
            ...
            <View style={styles.cardMainMessage}>
              <Text style={styles.textMainMessage}>Eventos</Text>
              <ScrollView contentContainerStyle={styles.scrollView}>
                {response.eventos &&
                  response.eventos.map((item, index) => (
                    <Surface key={index} style={styles.cardDetails}>
                      <Surface style={styles.dateDetails}>
                        <Text style={styles.titleTextDetails}>Fecha:</Text>
                        <Text style={styles.subtitleTextDetails}>
                          {DateTime.fromISO(item.fecha.toString()).toFormat(
                            'dd/MM/yyyy',
                          )}
                        </Text>
                      </Surface>
          ...
          <View style={{...styles.cardMainMessage, marginVertical: 20}}>
            <Icon
            ...
            />
            <Text style={{textAlign: 'center'}}>
              Probablemente tu paquete aún no ha llegado a El Salvador, intenta
              más tarde.
            </Text>
          </View>
        ) : (
